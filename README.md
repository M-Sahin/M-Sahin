# Hi, welcome to my Github profile ! 👋

Full‑stack developer | AI enthusiast | Lifelong learner

[![GitHub followers](https://img.shields.io/github/followers/M-Sahin?label=Follow&style=social)](https://github.com/M-Sahin)
[![Website](https://img.shields.io/badge/portfolio-website-blue?style=flat&logo=firefox)](https://msahin-dev.vercel.app/)
---

I'm a passionate .NET Engineer with over 5 years of experience building scalable web applications and enterprise solutions. My expertise lies in creating robust backend systems, implementing clean architecture patterns, and leading development teams to deliver high-quality software. I also have a great passion for building and designing clean and minimalistic front-end interfaces.

When I'm not coding, you'll find me exploring new technologies, contributing to open-source projects, or mentoring aspiring developers. I believe in writing clean, maintainable code and fostering collaborative team environments where everyone can grow and succeed.
---

## 🔭 What I'm working on
- Improving my portfolio and blogging about lessons learned

## 🌱 I’m learning
- Cloud-native patterns, advanced TypeScript types, and observability best practices.
- AI driven technology, like RAG Agents.
- Making visually appealing front-end designs.

## 👯 I’m open to
Collaboration on developer tools, mentoring, and interesting React / Node.js and .NET projects.

## 🛠️ Tech & Tools
- Languages: JavaScript (ES6+), TypeScript, Python, C#
- Frameworks: React, Next.js, Node.js, Express, .NET
- DevOps: Docker, GitHub Actions, Kubernetes (basic)
- Databases: PostgreSQL, Redis

## ✨ Featured projects
- [awesome-repo](https://github.com/M-Sahin/OmgekeerdeStemWijzer.Api/tree/main) — A RAG Agent API built entirely in .NET!

---

## 📫 How to reach me
- Email: murat.sa25@gmail.com
- LinkedIn: https://www.linkedin.com/in/murat-s-70545987/
---

Thanks for stopping by — feel free to say hi 👋
